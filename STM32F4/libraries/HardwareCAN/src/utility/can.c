...
#define CAN_INAK_TimeOut ((uint32)0x00FFFFFF)

/* Time out for SLAK bit */
#define CAN_SLAK_TimeOut ((uint32)0x00FFFFFF)
...
#define CAN_CLOCK (42000000UL)

static const struct can_speed_info can_speed_table[] = {
	[CAN_SPEED_10] = {.btr = (((1 - 1) << CAN_BTR_SJW_POS) |
							  ((16 - 1) << CAN_BTR_TS1_POS) |
							  ((3 - 1) << CAN_BTR_TS2_POS) |
							  (CAN_CLOCK / 10000UL / 20UL - 1) //20 is number of timequantas
							  )},
	[CAN_SPEED_125] = {.btr = (((1 - 1) << CAN_BTR_SJW_POS) |
							   ((13 - 1) << CAN_BTR_TS1_POS) |
							   ((2 - 1) << CAN_BTR_TS2_POS) |
							   (CAN_CLOCK / 125000UL / 16UL - 1) //16 is number of timequantas
							   )},
	[CAN_SPEED_250] = {.btr = (((1 - 1) << CAN_BTR_SJW_POS) |
							   ((11 - 1) << CAN_BTR_TS1_POS) |
							   ((2 - 1) << CAN_BTR_TS2_POS) |
							   (CAN_CLOCK / 250000UL / 14UL - 1))},
	[CAN_SPEED_500] = {.btr = (((1 - 1) << CAN_BTR_SJW_POS) |
							   ((11 - 1) << CAN_BTR_TS1_POS) |
							   ((2 - 1) << CAN_BTR_TS2_POS) |
							   (CAN_CLOCK / 500000UL / 14UL - 1))},
   [CAN_SPEED_500_2] = {.btr = (((1 - 1) << CAN_BTR_SJW_POS) |
							   ((14 - 1) << CAN_BTR_TS1_POS) |
							   ((6 - 1) << CAN_BTR_TS2_POS) |
							   (CAN_CLOCK / 500000UL / 21UL - 1))},
	[CAN_SPEED_1000] = {.btr = (((1 - 1) << CAN_BTR_SJW_POS) |
								((14 - 1) << CAN_BTR_TS1_POS) |
								((6 - 1) << CAN_BTR_TS2_POS) |
								(CAN_CLOCK / 1000000UL / 21UL - 1))}};
...
uint32 cnt_fifo0_over=0;
uint32 cnt_fifo1_over=0;
uint32 cnt_fifo0_full=0;
uint32 cnt_fifo1_full=0;

uint16 rx_isr_count=0;
...
CAN_STATUS can_deinit(CAN_Port *CANx)
{
	if (CANx == CAN1_BASE)
	{
		nvic_irq_disable(NVIC_CAN1_RX0); // Disable interrupts
		nvic_irq_disable(NVIC_CAN1_RX1);
		nvic_irq_disable(NVIC_CAN1_TX);
		...
		rcc_reset_dev(RCC_CAN1);
		rcc_clk_disable(RCC_CAN1);
		can_active = 0;
	}
...
CAN_STATUS can_init(CAN_Port *CANx, uint32 control, uint8 speed)
{
	...
	nvic_irq_enable(NVIC_CAN1_RX0); // Enable interrupts
	nvic_irq_enable(NVIC_CAN1_RX1);
	nvic_irq_enable(NVIC_CAN1_TX);

	rcc_clk_enable(RCC_CAN1); // and CAN
	rcc_reset_dev(RCC_CAN1);  // reset CAN interface
	...
	if (can_init_leave(CANx) == CAN_OK)
	{
		...
		while (!(CANx->TSR & CAN_TSR_TME1))
			; // Transmit mailbox 0 is empty
		while (!(CANx->TSR & CAN_TSR_TME2))
			; // Transmit mailbox 0 is empty
	}
	return status;
}
...
{	
	...
	if (CANx == CAN1_BASE)
	{
		...
		case CAN_GPIO_PB8_PB9:
			...
			gpio_set_af_mode(PB8, GPIO_AFMODE_CAN1_2);
			gpio_set_af_mode(PB9, GPIO_AFMODE_CAN1_2);
			gpio_set_mode(PB8, GPIO_AF_INPUT);
			gpio_set_mode(PB9, GPIO_AF_OUTPUT_PP);
			break;		
#if NR_GPIO_PORTS >= 4
		case CAN_GPIO_PD0_PD1:
			rcc_clk_enable(RCC_GPIOD);
			gpio_set_af_mode(PD0, GPIO_AFMODE_CAN1_2);
			gpio_set_af_mode(PD1, GPIO_AFMODE_CAN1_2);
			gpio_set_mode(PD0, GPIO_AF_INPUT);
...
CAN_TX_MBX can_transmit(CAN_Port *CANx, CanMsg *msg)
{
	...
	CANx->sTxMailBox[mbx].TDLR = (((uint32)msg->Data[3] << 24) |
								  ((uint32)msg->Data[2] << 16) |
								  ((uint32)msg->Data[1] << 8) |
								  ((uint32)msg->Data[0]));
	CANx->sTxMailBox[mbx].TDHR = (((uint32)msg->Data[7] << 24) |
...
CAN_STATUS can_tx_status(CAN_Port *CANx, CAN_TX_MBX mbx)
{
	...
	{
	case CAN_TX_MBX0:
		tmp = (uint8)((CANx->TSR & CAN_TSR_RQCP0) << 2); 
		tmp2 = (uint8)((CANx->TSR & CAN_TSR_RQCP0) >>0);
		state |= (uint8)((CANx->TSR & CAN_TSR_RQCP0) << 2);
		state |= (uint8)((CANx->TSR & CAN_TSR_TXOK0) >> 0);
		state |= (uint8)((CANx->TSR & CAN_TSR_TME0) >> 26);
		break;
	case CAN_TX_MBX1:
		tmp = (uint8)((CANx->TSR & CAN_TSR_RQCP1) >> 6);
		tmp2 = (uint8)((CANx->TSR & CAN_TSR_RQCP1) >> 8);
...
{
	nvic_irq_disable(NVIC_CAN1_RX0);
	nvic_irq_disable(NVIC_CAN1_RX1);
	__asm volatile("dsb");						 // Synchronization of the pipeline to guarantee
	__asm volatile("isb");						 // that the protection against interrupts is effective
	can_rx_head = can_rx_tail = can_rx_lost = 0; // JMD 2017/07/18
	nvic_irq_enable(NVIC_CAN1_RX0);
	nvic_irq_enable(NVIC_CAN1_RX1);
}
...
void can_rx_queue_free(void)
{
	if (can_rx_head != can_rx_tail) // JMD 2017/07/18 -- changed
	{
		nvic_irq_disable(NVIC_CAN1_RX0); // JMD atomicity problem
		nvic_irq_disable(NVIC_CAN1_RX1);
		__asm volatile("dsb"); // Synchronization of the pipeline to guarantee
		__asm volatile("isb"); // that the protection against interrupts is effective
		can_rx_tail = (can_rx_tail == (CAN_RX_QUEUE_SIZE - 1)) ? 0 : (can_rx_tail + 1);
		nvic_irq_enable(NVIC_CAN1_RX0); // fin JMD problème d'atomicité
		nvic_irq_enable(NVIC_CAN1_RX1);
	}
}
...
CanMsg *can_read(CAN_Port *CANx, CAN_FIFO fifo, CanMsg *msg)
{
	...
	msg->TIME = (uint16)(CANx->sFIFOMailBox[fifo].RDTR >> 16);
...
void can_err_status(CAN_Port *CANx, CanErrStatus* status) {
	uint32_t esr = CANx->ESR;
	status->BUSOFF=(esr >> 2) & 0x1;
	status->ERR_PASSIVE=(esr >> 1) & 0x1;
	status->ERR_WARNING=(esr >> 0) & 0x1;
	status->LEC= (esr >> 4) & 0x7;
	status->TEC= (esr >> 16) & 0xFF;
	status->REC= (esr >> 24 ) & 0xFF;
	status->FIFO0_FULL_COUNT=cnt_fifo0_full;
	status->FIFO0_OVERRUN_COUNT=cnt_fifo0_over;
	status->FIFO1_FULL_COUNT=cnt_fifo1_full;
	status->FIFO1_OVERRUN_COUNT=cnt_fifo1_over;
}

uint16 can_rx_isr_count() {
	return rx_isr_count;
}

void CAN_SCE_IRQ_Handler(void) {
	uint32_t esr = CAN1_BASE->ESR;
...
uint8 CAN_RX0_IRQ_Handler(void)
{
	...
	if (can_active)
	{
		if (CAN1_BASE->RF0R >> 3 & 0x1) ++cnt_fifo0_full;
		if (CAN1_BASE->RF0R >> 4 & 0x1) ++cnt_fifo0_over;
		if (CAN1_BASE->RF1R >> 3 & 0x1) ++cnt_fifo0_full;
		if (CAN1_BASE->RF1R >> 4 & 0x1) ++cnt_fifo0_over;
